...
BEGIN {
  my %method_for = (
    ...
    map { $_ => $_ } qw/install_sets install_path original_prefix install_base_relpaths prefix_relpaths _default_install_paths prefix_relative install_destination install_types install_map installdirs install_base prefix blib create_packlist destdir/,
  );
  for my $local (keys %method_for) {
    ...
    *{$local} = sub {
      my $self = shift;
      return $self->installpaths->$remote(@_);
    };
  }
}
